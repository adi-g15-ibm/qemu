...
static target_ulong pnv_handle_sprd_load(CPUPPCState *env)
{
...
static void pnv_handle_sprd_store(CPUPPCState *env, target_ulong val)
{
    PowerPCCPU *cpu = env_archcpu(env);
    target_ulong sprc = env->spr[SPR_POWER_SPRC];
    PnvCore *pc = pnv_cpu_state(cpu)->pnv_core;
...
static void pnv_chip_power9_realize(DeviceState *dev, Error **errp)
{
    ...
    PowerPCCPU *cpu = chip->cores[0]->threads[0];
    CPUState *cs = CPU(cpu);
    PowerPCCPUClass *cpu_class = POWERPC_CPU_GET_CLASS(cs);

    cpu_class->handle_sprd_load = pnv_handle_sprd_load;
    cpu_class->handle_sprd_store = pnv_handle_sprd_store;
