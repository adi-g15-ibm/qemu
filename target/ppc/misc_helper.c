...
target_ulong helper_load_sprd(CPUPPCState *env)
{
    ...
    if (pcc->handle_sprd_load) {
        return pcc->handle_sprd_load(env);
    }

    /*
     * Above handler is only implemented for PowerNV, return in case of
     * PowerVM
     */
    return 0;
}
...
void helper_store_sprd(CPUPPCState *env, target_ulong val)
{
    ...
    if (pcc->handle_sprd_store) {
        return pcc->handle_sprd_store(env, val);
    }
}
